/**
 * Producto.java
 * Proporciona funcionalidades como consultar y mostrar productos en una tabla, buscar productos, 
...
public class Producto extends javax.swing.JPanel {
    ...
    public static String productoUpdate = "";
    ...
    private void initStyles() {
        ...
        tituloProducto.putClientProperty("FlatLaf.styleClass", "h1");
        tituloProducto.setForeground(new java.awt.Color(90, 58, 201));

        // Texto dentro de los campos, indicando la información que se debe introducir.
        search.putClientProperty("JTextField.placeholderText", "Ingrese el producto que desea buscar.");
    ...
    private void propertyColumnTable() {
        ...
        tablaProducto.getColumnModel().setColumnMargin(35);

        // Alto de filas
        tablaProducto.setRowHeight(22);

        // Ancho de columnas
        tablaProducto.getColumnModel().getColumn(0).setPreferredWidth(2);
        tablaProducto.getColumnModel().getColumn(0).setResizable(false);

        tablaProducto.getColumnModel().getColumn(1).setPreferredWidth(61);
        tablaProducto.getColumnModel().getColumn(1).setResizable(false);

        tablaProducto.getColumnModel().getColumn(2).setPreferredWidth(46);
        tablaProducto.getColumnModel().getColumn(2).setResizable(false);

        tablaProducto.getColumnModel().getColumn(3).setPreferredWidth(43);
        tablaProducto.getColumnModel().getColumn(3).setResizable(false);

        tablaProducto.getColumnModel().getColumn(4).setPreferredWidth(38);
        tablaProducto.getColumnModel().getColumn(4).setResizable(false);

        tablaProducto.getColumnModel().getColumn(5).setPreferredWidth(2);
        tablaProducto.getColumnModel().getColumn(5).setResizable(false);

        tablaProducto.getColumnModel().getColumn(6).setPreferredWidth(43);
    ...
    private void consultaTable() {
        try {
            ...
            PreparedStatement pst = cn.prepareStatement("SELECT codigo_prod, barcode, nombre, precio,"
                    + "stock, cantidad, fecha_creacion FROM producto ORDER BY codigo_prod");

            ResultSet rs = pst.executeQuery();

            tablaProducto = new JTable(model);
            ...
            while (rs.next()) {
                ...
                // Obtener el precio y agregar " Bs" al final
                double precio = rs.getDouble(4);
                fila[3] = String.format("%.2f Bs", precio);
    ...
    private boolean eliminarUsuarios(List<String> codigo) {
        try {
            Connection cn = Conexion.conectar();
            StringBuilder consulta = new StringBuilder();
            consulta.append("DELETE FROM producto WHERE codigo_prod IN (");
            ...
            for (int i = 0; i < codigo.size(); i++) {
                pst.setString(i + 1, codigo.get(i));
            }
    ...
    private void initComponents() {
        ...
        tablaProducto.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Código", "Barcode", "Nombre", "Precio", "Stock", "Cantidad", "Fecha de creación"
            }
        ) {
    ...
    private void botonBorrarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonBorrarActionPerformed
        ...
        int[] filasSeleccionadas = tablaProducto.getSelectedRows();

        // Verifica si hay filas seleccionadas
        if (filasSeleccionadas.length == 0) {
            JOptionPane.showMessageDialog(null, "Selecciona al menos un producto para eliminar.",
        ...
        if (filasSeleccionadas.length == 1) {
            mensajeConfirmacion = "¿Está seguro(a) de eliminar el producto seleccionado?";
        } else {
            mensajeConfirmacion = "¿Está seguro(a) de eliminar los " + filasSeleccionadas.length + " productos seleccionados?";
        }
        ...
        if (opcion == JOptionPane.YES_OPTION) {
            ...
            for (int filaSeleccionada : filasSeleccionadas) {
                String codigo1 = tablaProducto.getValueAt(filaSeleccionada, 0).toString();
                codigo.add(codigo1);
            }

            // Realiza la eliminación de las filas en la base de datos
            if (eliminarUsuarios(codigo)) {
    ...
    private void botonEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonEditarActionPerformed
        ...
        if (filaSeleccionada != -1) {
            ...
            String codigo = tablaProducto.getValueAt(filaSeleccionada, 0).toString();
            String barcode = tablaProducto.getValueAt(filaSeleccionada, 1).toString();
            String nombre = tablaProducto.getValueAt(filaSeleccionada, 2).toString();
            String precio = tablaProducto.getValueAt(filaSeleccionada, 3).toString();
            String stock = tablaProducto.getValueAt(filaSeleccionada, 4).toString();
            String cantidad = tablaProducto.getValueAt(filaSeleccionada, 5).toString();

            // Realiza una consulta adicional para obtener la contraseña del usuario.
            try {
                Connection cn = Conexion.conectar();
                PreparedStatement pst = cn.prepareStatement("SELECT descripcion FROM producto WHERE codigo_prod = ?");
                ...
                if (rs.next()) {
                    ...
                    EditProducto editProductoPanel = new EditProducto();
                    editProductoPanel.setProductoSeleccionado(barcode); // Obtiene el barcode de la tabla dentro de los parentesis.
                ...
                System.err.println("Error al obtener la descripción del producto: " + e);
                JOptionPane.showMessageDialog(null, "¡Error al obtener la descripción del producto! "
                        + "Póngase en contacto con el Administrador.", "Error", JOptionPane.ERROR_MESSAGE);
            }

        } else {
            JOptionPane.showMessageDialog(null, "Seleccione un producto de la tabla.", "Advertencia",
    ...
    private void botonBuscarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botonBuscarActionPerformed
        ...
        try {
            ...
            String consulta = "SELECT * FROM producto WHERE codigo_prod LIKE ? OR barcode LIKE ? OR nombre LIKE ? "
                    + "OR precio LIKE ? OR stock LIKE ? OR cantidad LIKE ? OR descripcion LIKE ? OR fecha_creacion LIKE ?";
            ...
            while (rs.next()) {
                String codigo_prod = rs.getString("codigo_prod");
                ...
                resultados.append("Código: ").append(codigo_prod).append("\n");
                resultados.append("Barcode: ").append(barcode).append("\n");
                resultados.append("Nombre: ").append(nombre).append("\n");
                resultados.append("Precio: ").append(precio).append(" Bs").append("\n");
                resultados.append("Stock: ").append(stock).append("\n");
                resultados.append("Cantidad: ").append(cantidad).append("\n");
                resultados.append("Descripción: ").append(descripcion).append("\n");
                resultados.append("Fecha de creación: ").append(fecha_creacion).append("\n\n");
            }
